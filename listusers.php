...
$i=1;
	if(isset($_POST['search']))
    {
        ...
        $query_search = "SELECT * FROM users WHERE firstName LIKE '$search%'
		OR lastName LIKE '$search%' OR username LIKE '$search%' OR status LIKE '$search%' OR email LIKE '$search%'";
        $st = mysqli_query($con,$query_search);
		}
		else
		{
$st=mysqli_query($con,"select * from users order by userid desc");
		}			   
...
?>
	<td><a class="btn btn-primary" href='addusers.php?editid=<?php echo $row ['userID'];?>'>
						   <span class="glyphicon glyphicon-wrench" aria-hidden="true"></span></a> 
                            <a  href='listusers.php?delid=<?php echo $row ['userID'];?>'onclick="return confirm('Are you sure you want to delete this item?');" class="btn btn-danger" ><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
